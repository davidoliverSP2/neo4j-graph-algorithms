...
It is possible to define preliminary labels (identifiers) of nodes using the `partition` parameter.
...
Algorithm first checks if there is a `seed label` assigned to the node, and loads it if there is one.
...
| write | boolean | true | yes | Specifies if the result was written back as a node property
|===

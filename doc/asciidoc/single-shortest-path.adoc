...
// tag::introduction[]
The Single Source Shortest Path (SSSP) algorithm calculates the shortest (weighted) path between a pair of nodes.
...
* First, we go by rail from A to D, at a cost of 50.
* Then, from D to E by road, for an additional 30.
* Finally, from E to F by rail, for a further 20.

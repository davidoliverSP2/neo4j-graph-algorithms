...
final class HugeBiMultiBitSet32 {
    ...
    private final FixedLongArray bits;
    private final FixedLongArray.Cursor cursor;
    ...
    HugeBiMultiBitSet32(long nodeCount, AllocationTracker tracker) {
        try {
            bits = FixedLongArray.newArray(nodeCount, tracker);
    ...
    long nextSetNodeId(long fromNodeId) {
        final FixedLongArray.Cursor cursor = bits.cursor(fromNodeId, this.cursor);
        long n = fromNodeId;
        while (cursor.next()) {
    ...
    boolean copyInto(final HugeMultiBitSet32 target) {
        ...
        final FixedLongArray.Cursor cursor = bits.cursor(0, this.cursor);
